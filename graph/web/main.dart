...
void main()
{
  ...
  HttpRequest.getString("../../graph.json").then((String data){
    Map input = JSON.decode(data);
...
class GraphVisualizer
{
  ...
  GraphVisualizer(Map input)
  {
    ...
    input["nodes"].forEach((String k, String v) {
      int kI = int.parse(k);
      drawNode(x, y, v, radius);
      nodes[kI] = new VisualizationNode(v,x,y);
      x+= paddingW;
      if((kI+1)%cols == 0)
      {
        y += paddingH+10;
  ...
  void drawEdge(VisualizationNode from, VisualizationNode to, int type)
  {
    ...
    drawArrow(from,to,filled,closed,color);
  }
